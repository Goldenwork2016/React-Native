...
class Publish extends Component {
  ...
  _validateForm () {
    if (!this.titleInputValue || this.titleInput == '') {
      return window.alert('The title can not be blank!')
    }

    if (!this.contentInputValue || this.contentInput == '') {
      return window.alert('You have not written anything yet!')
    }

    if (this.titleInputValue.length <= 10) {
      return window.alert('The number of words must be more than 10 words!')
    }
  ...
  _submit () {
    ...
    actions.publish({
      ...
      resolved: () => {
        actions.toast('Successful release!')
        router.pop()
      },
      rejected: () => {
        actions.toast('Release failed')
      }
    })
  }
      ...
      <View style={styles.container}>
        ...
        <ScrollView
          ...
          <TextInput
            ...
            placeholder='Please enter the post contents here...'
					/>
        </ScrollView>
